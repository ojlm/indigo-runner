...
async function runKarateTest(args = null) {
  ...
  if (!runFilePath.toLowerCase().endsWith(standaloneBuildFile)) {
    if (Boolean(vscode.workspace.getConfiguration('IndigoRunner.buildSystem').get('useWrapper'))) {
    ...
    if (Boolean(vscode.workspace.getConfiguration('IndigoRunner.buildDirectory').get('cleanBeforeEachRun'))) {
      runPhases = "clean test"
    } else {
      runPhases = "test"
    }
    let karateRunnerArgs = String(vscode.workspace.getConfiguration('IndigoRunner.karateRunner').get('commandLineArgs'))
    if (Boolean(vscode.workspace.getConfiguration('IndigoRunner.karateCli').get('overrideKarateRunner'))) {
      let karateCliArgs = String(vscode.workspace.getConfiguration('IndigoRunner.karateCli').get('commandLineArgs'))
      ...
      if (Boolean(vscode.workspace.getConfiguration('IndigoRunner.karateRunner').get('promptToSpecify'))) {
        karateRunner = await vscode.window.showInputBox({
          prompt: "Indigo Runner",
          value: String(vscode.workspace.getConfiguration('IndigoRunner.karateRunner').get('default'))
        })
        if (karateRunner !== undefined && karateRunner !== "") {
          await vscode.workspace.getConfiguration().update('IndigoRunner.karateRunner.default', karateRunner)
        }
      } else {
        karateRunner = String(vscode.workspace.getConfiguration('IndigoRunner.karateRunner').get('default'))
      }
      if (karateRunner === undefined || karateRunner === "") {
        return
      }
    ...
    let karateJarArgs = String(vscode.workspace.getConfiguration('IndigoRunner.karateJar').get('commandLineArgs'))
    if (karateJarArgs === undefined || karateJarArgs === "") {
      return
    }
    runCommand = `${karateJarArgs} "${karateJarOptions}"`
  }
  let relativePattern = new vscode.RelativePattern(projectRootPath, String(vscode.workspace.getConfiguration('IndigoRunner.reports').get('toTargetByGlob')))
...
function displayReportsTree(displayType) {
  vscode.workspace.getConfiguration().update('IndigoRunner.reports.activityBarDisplayType', displayType)
}
  ...
  let filterByGlob = async () => {
    ...
    try {
      await new Promise<string>((resolve) => {
        ...
        inputBox.value = String(vscode.workspace.getConfiguration('IndigoRunner.reports').get('toTargetByGlob'))
        ...
        disposables.push(
          inputBox.onDidTriggerButton((item) => {
            if (item === resetButton) {
              inputBox.value = context.extension.packageJSON.contributes.configuration.properties['IndigoRunner.reports.toTargetByGlob'].default
            }
          }),
          inputBox.onDidAccept(async () => {
            if (initialValue.trim() != inputBox.value.trim()) {
              ...
              await new Promise(resolve => {
                ...
                vscode.workspace.getConfiguration().update('IndigoRunner.reports.toTargetByGlob', inputBox.value)
              })
            }
...
function displayTestsTree(displayType) {
  vscode.workspace.getConfiguration().update('IndigoRunner.tests.activityBarDisplayType', displayType)
}
  ...
  let filterByGlob = async () => {
    ...
    try {
      await new Promise<string>((resolve) => {
        ...
        inputBox.value = String(vscode.workspace.getConfiguration('IndigoRunner.tests').get('toTargetByGlob'))
        ...
        disposables.push(
          inputBox.onDidTriggerButton((item) => {
            if (item === resetButton) {
              inputBox.value = context.extension.packageJSON.contributes.configuration.properties['IndigoRunner.tests.toTargetByGlob'].default
            }
          }),
          inputBox.onDidAccept(async () => {
            if (initialValue.trim() != inputBox.value.trim()) {
  ...
  let filterByTag = async () => {
    ...
    try {
      await new Promise<string>((resolve) => {
        ...
        inputBox.step = 2
        inputBox.totalSteps = 2
        inputBox.value = String(vscode.workspace.getConfiguration('IndigoRunner.tests').get('toTargetByTag'))
        ...
        disposables.push(
          inputBox.onDidTriggerButton((item) => {
            ...
            if (item === resetButton) {
              inputBox.value = context.extension.packageJSON.contributes.configuration.properties['IndigoRunner.tests.toTargetByTag'].default
            }
          }),
          inputBox.onDidAccept(async () => {
            if (initialValue.trim() != inputBox.value.trim()) {
              ...
              await new Promise(resolve => {
                ...
                vscode.workspace.getConfiguration().update('IndigoRunner.tests.toTargetByTag', inputBox.value)
              })
            }
            inputBox.enabled = true
            inputBox.busy = false
