...
export class HttpClient {
  ...
  get<T = any>(url: string, config?: axios.AxiosRequestConfig): Promise<T> {
    return new Promise<T>((reslove, reject) => {
      this.instance.get<T, axios.AxiosResponse<T>>(url, config).then(res => reslove(res.data)).catch(err => reject(err))
    })
  }

  delete<T = any>(url: string, config?: axios.AxiosRequestConfig): Promise<T> {
    return new Promise<T>((reslove, reject) => {
      this.instance.delete<T, axios.AxiosResponse<T>>(url, config).then(res => reslove(res.data)).catch(err => reject(err))
    })
  }

  head<T = any>(url: string, config?: axios.AxiosRequestConfig): Promise<T> {
    return new Promise<T>((reslove, reject) => {
      this.instance.head<T, axios.AxiosResponse<T>>(url, config).then(res => reslove(res.data)).catch(err => reject(err))
    })
  }

  options<T = any>(url: string, config?: axios.AxiosRequestConfig): Promise<T> {
    return new Promise<T>((reslove, reject) => {
      this.instance.options<T, axios.AxiosResponse<T>>(url, config).then(res => reslove(res.data)).catch(err => reject(err))
    })
  }

  post<T = any>(url: string, data?: any, config?: axios.AxiosRequestConfig): Promise<T> {
    return new Promise<T>((reslove, reject) => {
      this.instance.post<T, axios.AxiosResponse<T>>(url, data, config).then(res => reslove(res.data)).catch(err => reject(err))
    })
  }

  put<T = any>(url: string, data?: any, config?: axios.AxiosRequestConfig): Promise<T> {
    return new Promise<T>((reslove, reject) => {
      this.instance.put<T, axios.AxiosResponse<T>>(url, data, config).then(res => reslove(res.data)).catch(err => reject(err))
    })
  }

  patch<T = any>(url: string, data?: any, config?: axios.AxiosRequestConfig): Promise<T> {
    return new Promise<T>((reslove, reject) => {
      this.instance.patch<T, axios.AxiosResponse<T>>(url, data, config).then(res => reslove(res.data)).catch(err => reject(err))
    })
  }
